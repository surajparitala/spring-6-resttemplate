...
class BeerClientMockTest {
    ...
    public static class TestConfig {
        ...
        @Bean
        OAuthClientInterceptor oAuthClientInterceptor(OAuth2AuthorizedClientManager clientManager, ClientRegistrationRepository clientRegistrationRepository) {
            return new OAuthClientInterceptor(clientManager, clientRegistrationRepository);
        }
    ...
    void setUp() throws JsonProcessingException {
        ClientRegistration springauth = clientRegistrationRepository.findByRegistrationId("springauth");

        OAuth2AccessToken token= new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, "test", Instant.MIN, Instant.MAX);

        when(manager.authorize(any())).thenReturn(new OAuth2AuthorizedClient(springauth, "test", token));
